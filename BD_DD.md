...
【並列処理について】
本システムでは、複数のCSVファイルに対する処理（ダウンロード、検証・加工、ZIP圧縮・アップロード）は、各ファイルまたはグループ単位で並列に実行される設計となっています。これにより、全体の処理時間を短縮し、スケーラビリティを向上させています。
- 並列処理の主な対象：
  - S3からのCSVファイルダウンロード
  - データ検証・加工
  - ZIP圧縮・S3アップロード
- 並列処理の実装例：Pythonのconcurrent.futures.ThreadPoolExecutorやProcessPoolExecutorを利用し、各ファイル/グループごとにタスクを分割して同時実行します。
- 並列処理の考慮点：
  - ファイルごとに独立した処理となるため、データの整合性やリソース競合が発生しないよう設計されています。
  - 並列数（ワーカー数）は環境やリソース状況に応じて調整可能です。


### 4.1 入出力ファイルの仕様
- 入力ファイル名は「日付_時分_グループ名.csv」とし、出力ファイルは「日付_グループ名.csv」でグループごとにまとめて出力するように仕様変更。
  - 例: 入力ファイル名 2025-06-12_9000_test.csv, 2025-06-12_9001_test.csv, ...
